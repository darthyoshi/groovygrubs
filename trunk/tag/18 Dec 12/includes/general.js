...
function rate(rating,recipeID,userID) {
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (count - data >= 0.25 && count - data < 0.75) {
                output += "                        --><img id='star' src='starbrightleft.png' style='width:10px;' alt='{$i} star' /><!--\n";
                output += "                        --><img id='star' src='stargrayright.png' style='width:10px;' alt='{$i} star' /><!--\n";
                count++;
            }

            for (; count <= 5 ; count++)
                output += "                        --><img id='star' src='stargray.png' alt='{$i} star' /><!--\n";

            output += "                    --></fieldset>\n";

            $("#avgRating").replaceWith(output);
            $("#top10").load(location.href+" #top10>*",'');
            $("#numRate").load(location.href+" #numRate>*",'');
        }
    });
